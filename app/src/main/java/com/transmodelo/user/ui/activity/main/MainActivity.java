...
import static com.transmodelo.user.MvpApplication.DATUM;
import static com.transmodelo.user.MvpApplication.RIDE_REQUEST;
import static com.transmodelo.user.MvpApplication.canGoToChatScreen;
import static com.transmodelo.user.MvpApplication.isCard;
import static com.transmodelo.user.MvpApplication.isCash;
import static com.transmodelo.user.MvpApplication.isChatScreenOpen;
...
import static com.transmodelo.user.common.Constants.Status.ARRIVED;
import static com.transmodelo.user.common.Constants.Status.COMPLETED;
import static com.transmodelo.user.common.Constants.Status.DROPPED;
import static com.transmodelo.user.common.Constants.Status.EMPTY;
import static com.transmodelo.user.common.Constants.Status.PICKED_UP;
import static com.transmodelo.user.common.Constants.Status.RATING;
import static com.transmodelo.user.common.Constants.Status.SEARCHING;
import static com.transmodelo.user.common.Constants.Status.SERVICE;
import static com.transmodelo.user.common.Constants.Status.STARTED;
import static com.transmodelo.user.data.SharedHelper.key.PROFILE_IMG;
import static com.transmodelo.user.data.SharedHelper.key.SOS_NUMBER;
...
import com.transmodelo.user.chat.ChatActivity;
import com.transmodelo.user.common.Constants;
import com.transmodelo.user.common.InfoWindowData;
import com.transmodelo.user.common.LocaleHelper;
import com.transmodelo.user.common.PolyUtil;
import com.transmodelo.user.data.SharedHelper;
import com.transmodelo.user.data.network.APIClient;
import com.transmodelo.user.data.network.ApiInterface;
...
import com.transmodelo.user.data.network.model.LatLngFireBaseDB;
...
import com.transmodelo.user.data.network.util.AppConstant;
import com.transmodelo.user.ui.activity.coupon.CouponActivity;
import com.transmodelo.user.ui.activity.help.HelpActivity;
import com.transmodelo.user.ui.activity.invite_friend.InviteFriendActivity;
import com.transmodelo.user.ui.activity.location_pick.LocationPickActivity;
import com.transmodelo.user.ui.activity.notification_manager.NotificationManagerActivity;
import com.transmodelo.user.ui.activity.payment.PaymentActivity;
import com.transmodelo.user.ui.activity.profile.ProfileActivity;
import com.transmodelo.user.ui.activity.wallet.WalletActivity;
import com.transmodelo.user.ui.activity.your_trips.YourTripActivity;
import com.transmodelo.user.ui.adapter.ImageAdapter;
import com.transmodelo.user.ui.fragment.RateCardFragment;
import com.transmodelo.user.ui.fragment.book_ride.BookRideFragment;
import com.transmodelo.user.ui.fragment.invoice.InvoiceFragment;
import com.transmodelo.user.ui.fragment.rate.RatingDialogFragment;
import com.transmodelo.user.ui.fragment.schedule.ScheduleFragment;
import com.transmodelo.user.ui.fragment.searching.SearchingFragment;
import com.transmodelo.user.ui.fragment.service.ServiceTypesFragment;
import com.transmodelo.user.ui.fragment.service_flow.ServiceFlowFragment;
...
public class MainActivity extends BaseActivity implements
    ...
    private Runnable r;
    private Handler h;
    PlacesClient mPlaceDetectionClient;
...
// Create a new Places client instance.
        ...
        imageSlider = (ViewPager) findViewById(R.id.pagerSlide);
//        getBanners();
    }
    ...
    public void onSuccess(DataResponse dataResponse) {
        ...
        if (CURRENT_STATUS.equals(STARTED)
            ...
            if (mProviderLocation == null) {
                mProviderLocation = FirebaseDatabase.getInstance(AppConstant.FIREBASE_URL).getReference()
    ...
    public void changeFlow(String status) {
        ...
        switch (status) {
            case EMPTY:
                ...
                CURRENT_STATUS = EMPTY;
                /*hidden because client don't needed*/
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.source:
                ...
                // getAutoComplete(Constants.LocationActions.SELECT_SOURCE);
                break;
            case R.id.destination:
                ...
                // getAutoComplete(Constants.LocationActions.SELECT_DESTINATION);
                break;
            case R.id.changeDestination:
                ...
                // getAutoComplete(Constants.LocationActions.CHANGE_DESTINATION);
                break;
    ...
    private void getAutoComplete(String status) {
        ...
        if (!TextUtils.isEmpty(status) && status.equalsIgnoreCase(Constants.LocationActions.SELECT_SOURCE)) {
            AUTOCOMPLETE_REQUEST_CODE = 1;
        } else if (!TextUtils.isEmpty(status) && status.equalsIgnoreCase(Constants.LocationActions.SELECT_DESTINATION)) {
            AUTOCOMPLETE_REQUEST_CODE = 2;
        } else if (!TextUtils.isEmpty(status) && status.equals(Constants.LocationActions.CHANGE_DESTINATION)) {
                ...
                locationResult.addOnCompleteListener(this, task -> {
                    if (task.isSuccessful() && task.getResult() != null) {
                        ...
                        mDefaultLocation = new LatLng(
                                Double.valueOf(SharedHelper.getKey(this, "latitude", "-25.2744")),
                                Double.valueOf(SharedHelper.getKey(this, "longitude", "133.7751"))
                        );
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_PICK_LOCATION) if (resultCode == AppCompatActivity.RESULT_OK) {
            ...
            if (RIDE_REQUEST.containsKey(SRC_ADD)
                ...
                Log.i("MainActivity", "onActivityResult: if SRC_ADD, DEST_ADD, EMPTY");
                imageSlider.setVisibility(View.GONE);
                CURRENT_STATUS = SERVICE;
                ...
                Log.i("MainActivity", "onActivityResult: origin: " + origin + ", destination: " + destination);
                drawRoute(origin, destination);
...
// Use the builder to create a FindCurrentPlaceRequest.
        ...
        if (isLocationPermissionGranted) {
            if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION)
                ...
                return;
            mPlaceDetectionClient.findCurrentPlace(request).addOnCompleteListener(response -> {
                if (response.isSuccessful() && response.getResult() != null && TextUtils.isEmpty(sourceTxt.getText().toString())) {

                    FindCurrentPlaceResponse likelyPlaces = response.getResult();
                    // System.out.println(likelyPlaces.toString());
                    List<PlaceLikelihood> placeLikelihood = likelyPlaces.getPlaceLikelihoods();//{
                    Log.i("TAG", String.format("Place '%s' has likelihood: %f",
                            placeLikelihood.get(0).getPlace().getAddress(),
                            placeLikelihood.get(0).getLikelihood()));
                    sourceTxt.setText(placeLikelihood.get(0).getPlace().getAddress());
                    RIDE_REQUEST.put(SRC_ADD, placeLikelihood.get(0).getPlace().getAddress());
                    RIDE_REQUEST.put(SRC_LAT, placeLikelihood.get(0).getPlace().getLatLng().latitude);
                    RIDE_REQUEST.put(SRC_LONG, placeLikelihood.get(0).getPlace().getLatLng().longitude);
    ...
    public void getBanners() {
        ...
        Call<ArrayList<BannerModel>> call1 = apiInterface.getBanners();
        call1.enqueue(new Callback<ArrayList<BannerModel>>() {
            @Override
            public void onResponse(Call<ArrayList<BannerModel>> call, Response<ArrayList<BannerModel>> response) {
                //  User user1 = response.body();
                if (response.isSuccessful()) {
                    ArrayList<BannerModel> responseBody = response.body();
                    if (responseBody != null && !responseBody.isEmpty()) {
                        setPager(responseBody);
                    }
                }
    ...
    private void setPager(ArrayList<BannerModel> responseBody) {
        adapterView = new ImageAdapter(MainActivity.this, responseBody);
