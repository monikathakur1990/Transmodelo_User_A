...
import com.transmodelo.user.ui.adapter.ServiceAdapter;
...
import static com.transmodelo.user.common.Constants.RIDE_REQUEST.SERVICE_TYPE;
import static com.transmodelo.user.data.SharedHelper.getKey;
import static com.transmodelo.user.data.SharedHelper.getProviders;
...
public class ServiceTypesFragment extends BaseFragment implements ServiceTypesIView {
    ...
    private ServiceListener mListener = pos -> {
        ...
        for (Provider provider : getProviders(Objects.requireNonNull(requireActivity())))
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.payment_type:
                ((MainActivity) Objects.requireNonNull(requireActivity())).updatePaymentEntities();
                    ...
                    if (response.body() != null) {
                        ...
                        if (isFromAdapter) {
                            ...
                            if (service != null) {
                                ...
                                ((MainActivity) Objects.requireNonNull(requireActivity())).changeFragment(bookRideFragment);
                            }
                        }
            ...
            try {
                AsyncTask.execute(() -> {
                    for (Service s : mServices) {
                        ...
                        Log.e("ServiceTypesFragment", "onSuccess: key: "+key );
                        Log.e("ServiceTypesFragment", "onSuccess: s.marker: "+s.getMarker());

                        if (!TextUtils.isEmpty(s.getMarker()))
                            if (isAdded())
                            if (TextUtils.isEmpty(getKey(requireActivity(), key))) {
                                Bitmap b = ((BaseActivity) requireActivity()).getBitmapFromURL(s.getMarker());
                                if (b != null)
                                    putKey(requireActivity(), key, ((BaseActivity) requireActivity()).encodeBase64(b));
                            }
